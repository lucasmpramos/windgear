...
function ShareDraftButton({ productId, className = '' }: ShareDraftButtonProps) {
  ...
  return (
    <button
      onClick={handleShare}
      disabled={copying}
      className={`inline-flex items-center gap-2 px-3 py-1.5 text-sm font-medium text-blue-600 bg-blue-50 rounded-lg hover:bg-blue-100 transition disabled:opacity-50 ${className}`}
    >
      {copying ? (
